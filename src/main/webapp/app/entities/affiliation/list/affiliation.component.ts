...
export class AffiliationComponent implements OnInit {
  ...
  trackId(index: number, item: IAffiliation): number {
    return item.id!;
  }

  delete(affiliation: IAffiliation): void {
    const modalRef = this.modalService.open(AffiliationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IAffiliation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
